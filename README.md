...
面向长期迭代项目的页面开发与设计说明书 skill。**手机与桌面使用同一套页面**：每个页面只有一份 HTML/路由实现，通过响应式 CSS 和交互适配不同宽度。最常用的场景是：做好单页两端适配，新增不同用途的多个 HTML 页面时保持同一产品的风格，以及在单个 HTML 页面中开发多个模块时保持统一的排版和组件语言。

helloUI 不生成一套固定视觉风格。它先查找项目已有的共享 CSS、变量、组件、页面壳层和设计决策，再完成页面实现。六份 Markdown 文档用于长期维护；附带的同步脚本只收集源码路径线索，不会凭代码猜测业务意图或自动改写手写设计规则。

## 适用场景

- 把现有 HTML 页面的桌面布局适配到手机宽度，仍维护同一份页面代码。
- 开发首页、列表页、详情页等不同用途的多个 HTML 页面；每个页面都适配两端，并复用导航、字体、颜色、间距和组件状态。
...
```text
$hello-ui 为当前项目开发同一份 HTML 页面，让它适配手机和桌面宽度，并沿用已有视觉风格。
```
...
**长期项目第一次引入模板时，可以先用一次 `$hello-ui` 初始化六份设计文档。**让 Codex 读取模板、共享样式和已有组件，把能确认的风格、组件规则与页面结构写进对应文档。不能从模板证实的设计意图标为 `[待确认]`；同步脚本本身只列源码路径，设计规则仍需 Codex 结合项目内容填写。

```text
$hello-ui 根据我放进项目的模板初始化六份设计文档。记录已有的风格、组件和页面结构；没有证据的设计意图标为待确认。
```

**之后不用每次修改都调用。**普通文案或小样式调整直接改即可。新增页面、增加较大模块、做同一页面的手机适配，或发现跨页风格不一致时，再用 `$hello-ui` 辅助实现与检查。只有项目级设计规则真正改变，才更新对应文档，不必每次重写六份。

```text
$hello-ui 基于现有模板新增功能模块，保持跨模块风格一致，并让同一页面适配手机和桌面。只更新真正变化的设计规则。
```
...
```text
$hello-ui 在同一份详情页 HTML 上完成手机和桌面适配。保留现有视觉语言，检查导航、内容顺序、主要操作、长文本和横向溢出；不要另建手机版页面，完成后对两个视口进行验证。
```

helloUI 会先检查现有 CSS、组件和相邻页面，再确定同一页面在手机与桌面宽度下的内容顺序、栏位、操作区与交互。通过流式布局、媒体查询或项目已有响应式机制实现；断点根据内容拥挤程度和项目既有规则决定。需要改变现行设计规则时才更新设计文档。

### 开发多个 HTML 页面

```text
$hello-ui 在现有站点中新增列表页和详情页。每页各有一份 HTML，并分别适配手机与桌面宽度；沿用首页的导航、字体、颜色、间距和按钮样式，检查跨页面风格一致性。
```

优先复用项目已有共享样式表、CSS 变量和组件。新页面应继承相同的页面壳层；列表页与详情页可以采用不同栏位和内容密度，但同义按钮、表单、状态反馈应保持统一。纯 HTML 项目不必为复用引入新框架。

这里的“多个 HTML 页面”是不同功能页面，**不是**为同一个页面分别建立 `desktop.html` 与 `mobile.html`。
...
**手机端和桌面端是否需要两个页面？** 不需要。一个页面保留同一份 HTML/路由，按宽度调整布局、顺序、导航呈现与交互。首页和详情页等不同用途才是不同 HTML 文件。
